  ...
  return (
    <div className="min-h-screen bg-background px-6 py-12 flex flex-col items-center justify-center">
      <div className="w-full max-w-3xl">
          ...
          <div className="flex items-center gap-3">
            {steps.map((item, index) => (
              <div key={item} className="flex items-center flex-1 last:flex-none">
                <div className={clsx(
                  'w-10 h-10 rounded-none flex items-center justify-center font-bold transition-all',
                  index <= stepIndex ? 'gradient-brand text-white shadow-lg' : 'bg-card text-brand-300 border-4 border-brand-200 shadow-lg'
                )}>
                  {index < stepIndex ? <Check className="w-5 h-5" /> : index + 1}
                </div>
                {index < steps.length - 1 && (
                  <div className={clsx('h-1 flex-1 mx-3 rounded-none', index < stepIndex ? 'gradient-brand' : 'bg-brand-100')} />
                )}
              </div>
            ))}
          </div>
        ...
        <div className="bg-card p-8 md:p-12 rounded-none shadow-xl border-4 border-brand-200 shadow-lg min-h-[520px] flex flex-col">
            ...
            <div className="flex items-center gap-3">
              {currentStep !== 'about' && (
                <button
                  onClick={handleSkip}
                  disabled={loading}
                  className="px-6 py-4 rounded-none font-bold text-slate-900 hover:bg-background transition-colors disabled:opacity-50"
                >
                  Skip
                </button>
              )}
...
function StepContent({
  ...
  if (step === 'about') {
    return (
      <div className="space-y-8 flex-1">
        <div>
          <h2 className="text-3xl font-bold text-black mb-2">Academic details</h2>
          <p className="text-black">Add your school and role-specific academic information.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-semibold text-black mb-2">{formData.role === 'professor' ? 'University' : 'College'}</label>
            <select
              value={formData.school_name}
              onChange={(event) => setFormData({ ...formData, school_name: event.target.value })}
              className="w-full px-4 py-3 rounded-none border-4 border-brand-200 shadow-lg focus:ring-2 focus:ring-brand-500 outline-none transition-all bg-card"
            >
  ...
  if (step === 'biography') {
    return (
      <div className="space-y-8 flex-1">
        <div>
          <h2 className="text-3xl font-bold text-black mb-2">Write your biography</h2>
        ...
        <textarea
          ...
          className="w-full px-5 py-4 rounded-none border-4 border-brand-200 shadow-lg focus:ring-2 focus:ring-brand-500 outline-none transition-all resize-none text-black"
  ...
  if (step === 'interests') {
    return (
      <div className="space-y-8 flex-1">
        <div>
          <h2 className="text-3xl font-bold text-black mb-2">Add your interests</h2>
  ...
  if (step === 'transfer-goals') {
    return (
      <div className="space-y-8 flex-1">
        <div>
          <h2 className="text-3xl font-bold text-black mb-2">Transfer goals</h2>
  ...
  return (
    <div className="space-y-8 flex-1">
      <div>
        <h2 className="text-3xl font-bold text-black mb-2">Add a profile image</h2>
        <p className="text-black">Choose an image from your files.</p>
      </div>
      <div className="flex flex-col items-center gap-6">
        <div className="w-36 h-36 rounded-none gradient-soft flex items-center justify-center border-4 border-brand-200 shadow-lg shadow-xl overflow-hidden">
        ...
        <label className="cursor-pointer bg-card border-4 border-brand-200 shadow-lg text-black px-6 py-3 rounded-none font-bold shadow-sm hover:shadow-md transition-all flex items-center gap-2">
